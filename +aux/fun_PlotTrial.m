...
dataload=load(file_name);
data=dataload.PlotData;
firings=dataload.firings.spks;
Params=dataload.params;
Params.Ext=dataload.stimulus_save.Ext;
...
if plot_currents
    ...
    subplot(np,1,1);
    PlotMembrane(Sim,nplot,vplot,firings,ind_plot,VTh);
    xlim(XLIM);
    % D. Plot currents
    % bandpass IPSC
    hicutoff=200; % hi-edge (Hz)
    srate=10000; % sampling rate (Hz)
    PSCPlot(Sim,Ext,iIplot,iEplot,iExtplot,hicutoff,srate,VTh,ind_plot,tau);
    xlim(XLIM);
    % SAVE PLOT
